...
class AttendanceSheet(models.Model):
    ...
    def _compute_sheet_total(self):
        ...
        for sheet in self:
            ...
            print(len(sheet.line_ids))
            sheet.tot_absence = sum([l.diff_time for l in absence_lines]) / 2 if len(sheet.line_ids) > 40 else sum([l.diff_time for l in absence_lines])
            sheet.no_absence = len(absence_lines) / 2 if len(sheet.line_ids) > 40 else len(absence_lines)
            # conmpute earlyout
    ...
    def get_attendances(self):
        for att_sheet in self:
            ...
            #     for cal in emp.contract_id.shift_schedule:
            #         if cal.start_date >= from_date and cal.end_date <= to_date:
            #             calendar_id = cal.hr_shift
            #         else:
