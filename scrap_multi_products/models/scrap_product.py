...
class ScrapProductsByQuantity(models.Model):
    ...
    scrap_line = fields.One2many('scrap.product.line', 'scrap_id', string='Order Lines', copy=True, states={
    ...
    location_id = fields.Many2one('stock.warehouse')
    ...
    #         self.src_location_id = self.env['stock.location'].search([
    #             ('scrap_location', '=', True),
    ...
    def action_get_stock_move_lines(self):
        ...
        moves = self.env['stock.move'].search(
            [('scrap_product_line_id', 'in', line_ids)])
...
class ScrapProductLine(models.Model):
    ...
    scrap_id = fields.Many2one('scrap.products.by.quantity', string='Order Reference',
                               ondelete='cascade', copy=False)
    product_id = fields.Many2one(
        'product.product', 'Product Name', ondelete='restrict', required=True)
    quantity = fields.Float(string='Scrap Quanity', required=True)
    available_qty = fields.Float(
        string='Available Quanity', compute="_get_available_qty", store=True)
    src_loc_id = fields.Many2one(
        'stock.location', string='Source Location', required=True, domain="[('usage', '=', 'internal'), ('company_id', 'in', [company_id, False])]")
    dest_loc_id = fields.Many2one(
        'stock.location', string="Scrap Location", required=True, domain="[('scrap_location', '=', True), ('company_id', 'in', [company_id, False])]")
    product_uom = fields.Many2one(
        'uom.uom', related='product_id.uom_id', string='UoM', required=True)
    lot_id = fields.Many2one(
        'stock.production.lot', 'Lot/Serial Number',
        domain="[('product_id', '=', product_id), ('company_id', '=', company_id)]", check_company=True, help="Lot/Serial Number of the product to scrap.")
    product_tracking = fields.Selection(related='product_id.tracking')

    company_id = fields.Many2one(related='scrap_id.company_id')

    _sql_constraints = [
        ('qty_gt_zero', 'CHECK (quantity>0.00)',
         'Product Quantity to be scrapped needs to be greater than 0.'),
    ]
    ...
    def get_quantity(self):
        ...
        if self.product_id and self.src_loc_id:
            quants = quant_obj.search(
                [('product_id', '=', self.product_id.id), ('location_id', '=', self.src_loc_id.id)])
