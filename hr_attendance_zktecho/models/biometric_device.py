...
class BiomtericDeviceInfo(models.Model):
    ...
    def download_attendance_oldapi(self):
        ...
        try:
            ...
            _logger.error(str(e))
            raise UserError("Error while fetching attendance, please check the parameters and network connections because of error %s" % str(e))
        if (attendance):
...
#                                     raise UserError('Please make sure you have properly configured employee contract in order to be able to fetch attendances')
    ...
    #                            -----------------------------------------------------------------------------
                        ...
                        raise UserError("Error while processing fetched attendance %s" % str(e))
                else:
